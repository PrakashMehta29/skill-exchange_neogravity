...
const AppState = {
    ...
    myTrades: [
        { id: 101, skillId: 1, skill: 'Python for Beginners', withUser: 'Priya', status: 'completed', durationMinutes: 45, date: '2023-10-15' },
        { id: 102, skillId: 5, skill: 'Watercolor Painting', withUser: 'Maya', status: 'completed', durationMinutes: 60, date: '2023-10-22' },
        { id: 103, skillId: 12, skill: 'Personal Budgeting', withUser: 'Neha', status: 'pending', durationMinutes: 0, date: '2023-10-25' }
    ],
    isNearbyOnly: false
};
...
const Views = {
        ...
        <div id="view-login" class="view active">
            <div style="flex: 1; display: flex; flex-direction: column; justify-content: center;">
                ...
                <div class="glass" style="padding: 24px; border-radius: var(--radius-lg); margin-bottom: 16px;">
                    <!-- Step 1: Request OTP -->
                    <form id="login-form" onsubmit="event.preventDefault(); simpleLogin();">
                        <div class="input-group">
                            <label>Email Address</label>
                            <input type="email" id="login-email" class="input-field" placeholder="you@example.com" autocomplete="username" required>
                        </div>
                        
                        <div class="input-group">
                            <label>Password</label>
                            <input type="password" id="login-password" class="input-field" placeholder="••••••••" autocomplete="current-password" required>
                        </div>

                        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 24px;">
                            <input type="checkbox" id="login-remember" checked style="accent-color: var(--primary-color); width: 16px; height: 16px; cursor: pointer;">
                            <label for="login-remember" style="font-size: 0.9rem; color: var(--text-secondary); cursor: pointer; margin: 0;">Remember me</label>
                        </div>
                        
                        <button type="submit" class="btn btn-primary" style="width: 100%;">
                            Log In <i class="ph-arrow-right"></i>
                        </button>
                    </form>
                </div>
    ...
    trades: `
        <div id="view-trades" class="view">
            ...
            <div style="padding: 0 24px; margin-bottom: 16px; text-align: center;">
                <button class="btn btn-secondary" style="width: 100%; border-color: var(--primary-color); color: var(--primary-color);" onclick="scanNearby()">
                    <i class="ph-radar text-gradient" style="font-size: 20px;"></i> Scan Nearby Traders
                </button>
            </div>
...
function renderMyProfile() {
    ...
    container.innerHTML = `
        <div class="glass" style="padding: 24px; border-radius: var(--radius-lg); text-align: center; margin-bottom: 24px;">
            ...
            ` : `
                <button class="btn btn-secondary" style="margin-top: 16px; border-color: var(--primary-color); color: var(--primary-color);" onclick="editProfile()">
                    <i class="ph-pencil-simple"></i> Edit Profile
                </button>
            `}
        </div>
        
        <div class="glass" style="padding: 24px; border-radius: var(--radius-lg); margin-bottom: 24px; text-align: left;">
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 16px;">
                <i class="ph-book-open-text text-gradient" style="font-size: 24px;"></i>
                <h3 style="margin: 0;">Learning History</h3>
            </div>
            
            <div style="display: flex; gap: 12px; margin-bottom: 20px;">
                <div style="flex: 1; background: var(--surface-color); padding: 12px; border-radius: var(--radius-md); text-align: center;">
                    <div style="font-size: 1.5rem; font-weight: 700; color: var(--primary-color);" id="history-total-sessions">0</div>
                    <div style="font-size: 0.8rem; color: var(--text-secondary);">Sessions</div>
                </div>
                <div style="flex: 1; background: var(--surface-color); padding: 12px; border-radius: var(--radius-md); text-align: center;">
                    <div style="font-size: 1.2rem; font-weight: 700; color: var(--primary-color); margin-top: 4px;" id="history-total-time">0m</div>
                    <div style="font-size: 0.8rem; color: var(--text-secondary);">Total Time</div>
                </div>
            </div>
    ...
    const completedTrades = AppState.myTrades.filter(t => t.status === 'completed');
    const totalMinutes = completedTrades.reduce((sum, t) => sum + (t.durationMinutes || 0), 0);
    const hours = Math.floor(totalMinutes / 60);
    const mins = totalMinutes % 60;
    const timeString = hours > 0 ? `${hours}h ${mins}m` : `${mins}m`;
    
    document.getElementById('history-total-sessions').innerText = completedTrades.length;
    document.getElementById('history-total-time').innerText = timeString;
    
    const historyContainer = document.getElementById('history-list-container');
    if(completedTrades.length === 0) {
        historyContainer.innerHTML = `<p style="text-align: center; color: var(--text-secondary); font-size: 0.9rem;">No completed sessions yet.</p>`;
    } else {
        historyContainer.innerHTML = completedTrades.map(t => `
            <div style="display: flex; justify-content: space-between; padding: 12px 0; border-bottom: 1px solid var(--surface-light);">
                <div>
                    <div style="font-weight: 600; font-size: 0.95rem;">${t.skill}</div>
                    <div style="font-size: 0.8rem; color: var(--text-secondary);">with ${t.withUser} • ${t.date || 'Recent'}</div>
                </div>
                <div style="text-align: right;">
                    <div style="color: var(--primary-color); font-weight: 600;">+${t.durationMinutes || 0}m</div>
                    <div style="font-size: 0.8rem; color: var(--success);"><i class="ph-check-circle-fill"></i></div>
                </div>
            </div>
        `).join('');
    }
...
window.editProfile = () => {
    ...
    document.getElementById('profile-skill').value = u.skill || '';
    
    document.getElementById('profile-phone-locked').value = u.phone || '';
    document.getElementById('profile-email-locked').value = u.email || '';
    
    if(u.avatarDataUrl) {
        setAvatarPreview(u.avatarDataUrl);
    }
...
window.simpleLogin = () => {
    ...
    setTimeout(() => {
        ...
        if (remember) {
            // In a real app, this would persist the auth token
...
window.detectLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            document.getElementById('profile-address').value = `Scanned Location (${lat}, ${lng})`;
...
window.scanNearby = () => {
    ...
    setTimeout(() => {
        ...
        // Simulate finding nearby traders
        AppState.myTrades = AppState.myTrades.map(t => ({...t, isNearby: true})); // Mock logic
        showToast('Found 3 nearby traders!', 'success');
        
        // You can update the UI or filter here if needed
        renderTrades();
    }, 3000);
};
...
function attachEventListeners() {
    document.getElementById('form-profile').addEventListener('submit', (e) => {
        ...
        AppState.user = { 
            ...AppState.user, 
            isGuest: false,
            firstName, lastName, name: firstName + ' ' + lastName, nickname, dob, age, address, skill, avatarDataUrl,
            level: 1, 
            coins: 100, 
            trialDaysLeft: 30,
            subscriptionStatus: 'trial'
